...
class PytorchQuantizer():
    def __init__(self, model, input_shape, concrete_args=None):
        ...
        if isinstance(input_shape, (tuple, list)) and all(isinstance(element, int) for element in input_shape):
            self.input_shape = [input_shape]        
    ...
    def print_tabular(self):
        node_specs = [[n.op, n.name, n.target, n.args, n.kwargs]
                      for n in self.nodes]
        logger.debug(tabulate(node_specs,
              headers=['\nopcode', '\nname', '\ntarget', '\nargs', '\nkwargs']))        
        ...
        for node in list(graph_module.graph.nodes):
            if node.op == 'placeholder':
                ...
                if isinstance(module, (nn.Conv2d, nn.Conv1d)):
                    ...
                    module.qbit = 8
                    module.add_module('activation_pre_process', module.qconfig.activation())
                    module.register_forward_pre_hook(activation_pre_hook)
                    module.add_module('activation_post_process', module.qconfig.output())
        ...
        for node in list(self.observed_model.graph.nodes):
            if node.op == 'placeholder':
                ...
                with self.observed_model.graph.inserting_after(node):
                    ...
                    new_node = self.observed_model.graph.call_module(node.name, node.args, node.kwargs)                  
                ...
                with self.observed_model.graph.inserting_after(prev_node):
                    ...
                    new_node = self.observed_model.graph.call_module('output', node.args, node.kwargs)            

                node.replace_input_with(prev_node, new_node)                    

        self.quanted_model = torch.fx.GraphModule(self.observed_model, self.observed_model.graph)
