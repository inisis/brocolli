...
class GRULayer(BaseLayer):
    ...
    def generate_node(self, name=None, params=None, attr_dict=None):
        if self._module.num_layers == 1:
            ...
            gru_block.generate_block(self.num_layers-1, in_names=in_names, out_names=out_names)
            self.node_post_process(gru_block)
...
class GRUBlock(BaseLayer):
    ...
    def generate_block(self, block_id, in_names, out_names):
        ...
        if self.num_outputs == 2:
            gru_cell.add_bottom_top(in_names=[in_names[0]], out_names=[self._source_node.name+"_"+str(block_id)+"_out", out_names[1]])
        else:
            gru_cell.add_bottom_top(in_names=[in_names[0]], out_names=[self._source_node.name+"_"+str(block_id)+"_out"])

        params = self.get_gru_params(block_id)

        gru_cell.generate_params(params, self._source_node.name+"_"+str(block_id))  
        if self.num_inputs != 1:
            gru_cell._in_names.append(self._source_node.name+"_"+str(block_id)+"_slice")       
