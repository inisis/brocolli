...
from converter.layers import *
from converter.data import voc, coco
import os
...
class SSD(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.loc_conv1(sources[0])
        loc.append(x)

        x = self.conf_conv1(sources[0])
        conf.append(x)

        x = self.loc_conv2(sources[1])
        loc.append(x)

        x = self.conf_conv2(sources[1])
        conf.append(x)

        x = self.loc_conv3(sources[2])
        loc.append(x)

        x = self.conf_conv3(sources[2])
        conf.append(x)

        x = self.loc_conv4(sources[3])
        loc.append(x)

        x = self.conf_conv4(sources[3])
        conf.append(x)

        x = self.loc_conv5(sources[4])
        loc.append(x)

        x = self.conf_conv5(sources[4])
        conf.append(x)

        x = self.loc_conv6(sources[5])
        loc.append(x)

        x = self.conf_conv6(sources[5])
        conf.append(x)
