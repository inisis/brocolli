    ...
    def gen_ir(self):
        for node in self.pytorch_graph.nodes:
            ...
            if node.op == 'placeholder':
                ...
                elif isinstance(module, (nn.ConstantPad1d, nn.ConstantPad2d, nn.ConstantPad3d)): 
                ...
                if function_name == "relu":
                ...
                elif function_name == "adaptive_avg_pool2d" or function_name == "adaptive_avg_pool1d":
                    ...
                    if isinstance(node.args[1], tuple):
                        if all(isinstance(function, slice) for function in node.args[1]):
                            params_slices = []
                            for idx, function in enumerate(node.args[1]):
                                if function.start is None and function.stop is None and function.step is None:
                                    ...
                                    end_ = function.stop if function.stop is not None else 1 # maybe a bug
                                    axes_ = idx
                                    step_ = function.step if function.step is not None else 1

                                    params_slice = [np.array([start_]), np.array([end_]), np.array([axes_]), np.array([step_])]
                    ...
                    params_clip = [np.array(node.kwargs['min_val']), np.array(node.kwargs['max_val'])]
                    ...
                    if 'max_pool2d' in node.name or 'max_pool1d' in node.name:
                        ...
                        raise NotImplementedError("function %s is not implemented" % (node.name))                                                                                                                                                                                                                                                               
