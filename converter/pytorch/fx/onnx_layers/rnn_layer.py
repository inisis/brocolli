...
class RNNLayer(BaseLayer):
    ...
    def gen_rnn_block_outnames(self, idx=0):
    ...
    def generate_node(self, name=None, params=None, attr_dict=None):
        if self.num_layers == 1:
            ...
            rnn_block.generate_block(0, in_names=in_names, out_names=out_names)
            ...
            for idx in range(1, self._module.num_layers - 1):
                ...
                out_names = self.gen_rnn_block_outnames(idx)
                rnn_block.generate_block(idx, in_names=in_names, out_names=out_names)
                self.node_post_process(rnn_block)
            ...
            out_names = self.gen_rnn_block_outnames(self.num_layers-1)
                
            rnn_block.generate_block(self.num_layers-1, in_names=in_names, out_names=out_names)
            self.node_post_process(rnn_block)
            ...
            concat_layer.add_bottom_top(in_names=[self._name+"_layer_"+str(idx)+"_out_last" for idx in range(self.num_layers)], out_names=[self._source_node.name+"_1"])            
            concat_layer.generate_node(name=self._source_node.name+"_concat", attr_dict={"axis": 0})
...
class RNNBlock(BaseLayer):
    ...
    def get_permute_weight(self, weight):
        params = weight.detach().numpy()

        return params

    def get_rnn_params(self, layer_index=0):
        params = [None] * 4
    ...
    def generate_block(self, block_id, in_names=None, out_names=None):
        if self.num_inputs != 1:
            ...
            slice_layer.add_bottom_top(in_names=[in_names[1]], out_names=[self._source_node.name+"_"+str(block_id)+"_slice"])
            params_slice = [
            ...
            slice_layer.generate_node(self._source_node.name + "_" + str(block_id) +"_slice", params_slice)
            self.node_post_process(slice_layer)

        rnn_cell = RNNCell(self._source_node, self._module, auto_gen=False)
        if self.num_outputs == 2:
            rnn_cell.add_bottom_top(in_names=[in_names[0]], out_names=[self._source_node.name+"_"+str(block_id)+"_out", out_names[1]])
        else:
            rnn_cell.add_bottom_top(in_names=[in_names[0]], out_names=[self._source_node.name+"_"+str(block_id)+"_out"])

        params = self.get_rnn_params(block_id)

        rnn_cell.generate_params(params, self._source_node.name+"_"+str(block_id))   
        if self.num_inputs != 1:
            rnn_cell._in_names.append(self._source_node.name+"_"+str(block_id)+"_slice")

        rnn_cell.generate_node(self._source_node.name+"_"+str(block_id))
