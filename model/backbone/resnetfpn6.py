...
def resnet18fpn6(cfg, **kwargs):
    ...
    model = ResNetFPN(BasicBlock, [2, 2, 2, 2, 2, 2], norm_type=cfg.norm_type,
                      **kwargs)
    if cfg.pretrained:
...
def resnet34fpn6(cfg, **kwargs):
    ...
    model = ResNetFPN(BasicBlock, [3, 4, 6, 3, 2, 2], norm_type=cfg.norm_type,
                      **kwargs)
    if cfg.pretrained:
        pattern = re.compile(r'^(.*bn\d\.(?:weight|bias|running_mean|running_var))$')  # noqa
        state_dict = model_zoo.load_url(model_urls['resnet34'])
...
def resnet50fpn6(cfg, **kwargs):
    """Constructs a ResNet-50 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNetFPN(Bottleneck, [3, 4, 6, 3, 2, 2], norm_type=cfg.norm_type,
                      **kwargs)
    if cfg.pretrained:
        pattern = re.compile(r'^(.*bn\d\.(?:weight|bias|running_mean|running_var))$')  # noqa
        state_dict = model_zoo.load_url(model_urls['resnet34'])
...
def resnet101fpn6(cfg, **kwargs):
    """Constructs a ResNet-101 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNetFPN(Bottleneck, [3, 4, 23, 3, 2, 2], norm_type=cfg.norm_type,
                      **kwargs)
    if cfg.pretrained:
        pattern = re.compile(r'^(.*bn\d\.(?:weight|bias|running_mean|running_var))$')  # noqa
        state_dict = model_zoo.load_url(model_urls['resnet34'])
...
def resnet152fpn6(cfg, **kwargs):
    """Constructs a ResNet-152 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNetFPN(Bottleneck, [3, 8, 36, 3, 2, 2], norm_type=cfg.norm_type,
                      **kwargs)
    if cfg.pretrained:
        pattern = re.compile(r'^(.*bn\d\.(?:weight|bias|running_mean|running_var))$')  # noqa
        state_dict = model_zoo.load_url(model_urls['resnet34'])
