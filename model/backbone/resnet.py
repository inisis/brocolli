...
def resnet18(cfg, **kwargs):
    ...
    model = ResNet(BasicBlock, [2, 2, 2, 2], norm_type=cfg.norm_type,
                   **kwargs)
    if cfg.pretrained:
...
def resnet34(cfg, **kwargs):
    ...
    model = ResNet(BasicBlock, [3, 4, 6, 3], norm_type=cfg.norm_type,
                   **kwargs)
    if cfg.pretrained:
        pattern = re.compile(r'^(.*bn\d\.(?:weight|bias|running_mean|running_var))$')  # noqa
        state_dict = model_zoo.load_url(model_urls['resnet34'])
...
def resnet50(cfg, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(Bottleneck, [3, 4, 6, 3], norm_type=cfg.norm_type,
                   **kwargs)
    if cfg.pretrained:
        pattern = re.compile(r'^(.*bn\d\.(?:weight|bias|running_mean|running_var))$')  # noqa
        state_dict = model_zoo.load_url(model_urls['resnet34'])
...
def resnet101(cfg, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(Bottleneck, [3, 4, 23, 3], norm_type=cfg.norm_type,
                   **kwargs)
    if cfg.pretrained:
        pattern = re.compile(r'^(.*bn\d\.(?:weight|bias|running_mean|running_var))$')  # noqa
        state_dict = model_zoo.load_url(model_urls['resnet34'])
...
def resnet152(cfg, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(Bottleneck, [3, 8, 36, 3], norm_type=cfg.norm_type,
                   **kwargs)
    if cfg.pretrained:
        pattern = re.compile(r'^(.*bn\d\.(?:weight|bias|running_mean|running_var))$')  # noqa
        state_dict = model_zoo.load_url(model_urls['resnet34'])
